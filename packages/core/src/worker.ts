...
class TempFSTJoinCoordinator {
  ...
  private async englishMorphologyJoin(prev: string, next: string): Promise<any> {
    const prevLower = prev.toLowerCase();
    const nextLower = next.toLowerCase();
    ...
    if (prevLower === 'a') {
      ...
      const startsWithVowelSound =
        (vowelSounds.test(nextLower) && !consonantSoundU.test(nextLower) && !consonantSoundE.test(nextLower) && !consonantSoundO.test(nextLower)) ||
        hasSilentH;
    ...
    const pronounVerbContractions: Record<string, Record<string, string>> = {
  ...
  private async englishInflection(stem: string, suffix: string): Promise<any> {
  ...
  private async englishNounPlural(stem: string, suffix: string): Promise<any> {
    const suffixLower = suffix.toLowerCase();
    let result = stem + suffix;
  ...
  private async englishDerivationalSuffix(stem: string, suffix: string): Promise<any> {
    const suffixLower = suffix.toLowerCase();
    let result = stem + suffix;
