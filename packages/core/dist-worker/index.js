...
function getStemmer(lang) {
    if (!stemmers[lang]) {
        ...
        if (!Snowball) {
            stemmers[lang] = {
                stem: (s) => `SNOWBALL_UNAVAILABLE_IN_BROWSER:${s}`
            };
        }
        else {
...
function pluralize(lemma, lang) {
    switch (lang) {
        case "en-US":
            if (/[^aeiou]y$/.test(lemma))
                return lemma.replace(/y$/, "ies");
        ...
        case "it-IT":
            if (lemma.endsWith("o"))
                return lemma.slice(0, -1) + "i";
            if (lemma.endsWith("a"))
                return lemma.slice(0, -1) + "e";
            return lemma + "i";
        case "fi-FI":
            return lemma + "t";
        default:
...
export let lastHfstUpRaw = [];
export let lastHfstDownRaw = [];
let hfstPackUrl = undefined;
export function configureMorphHfst(options = {}) {
    if (options.wasmUrl)
        hfstWasmUrl = options.wasmUrl;
    if (options.packUrl)
        hfstPackUrl = options.packUrl;
}
let tagOrderPolicy = 'flexible';
...
export function configureTagOrdering(policy) {
    tagOrderPolicy = policy;
}
function orderTagsForGenerate(tags) {
    if (tagOrderPolicy === 'strict')
        return tags.slice(); // preserve input order
...
export function configureMorphRuntime(mode) {
